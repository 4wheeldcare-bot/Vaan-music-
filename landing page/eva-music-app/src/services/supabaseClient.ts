...
const supabaseNotConfiguredError = {
  message: 'Database is disabled in this build. The app is running in local-only mode.',
};
...
const noopAuth = {
  ...
  async signInWithPassword() {
    return { data: { user: null }, error: supabaseNotConfiguredError };
  },
  async signUp() {
    return { data: { user: null }, error: supabaseNotConfiguredError };
  },
  async signInWithOAuth() {
    return { data: null, error: supabaseNotConfiguredError };
  },
  async signOut() {
    return { error: null };
  },
